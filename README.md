...
Между попытками crawler делает небольшую паузу. Если задан `--delay`, это значение используется как задержка между повторными попытками. Если `--delay` не задан, используется небольшая задержка по умолчанию.
