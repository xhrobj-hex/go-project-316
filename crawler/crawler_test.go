...
func TestAnalyze_ReturnsEmptyCollectionsForErrorPage(t *testing.T) {
	const mockedPageURL = mockedBaseURL + "/index.html"

	client := &http.Client{
		Transport: roundTripFunc(func(rq *http.Request) (*http.Response, error) {
			return newTestResponse(rq, http.StatusInternalServerError, "500 Internal Server Error", "server error"), nil
		}),
	}
	...
	if report.Pages[0].Assets == nil {
		t.Fatal("got assets nil, want empty slice")
	}

	if report.Pages[0].BrokenLinks == nil {
		t.Fatal("got broken links nil, want empty slice")
	}
}
